...
int extractNumber(char line[]){
	int x = 0;
	char s[10]; /*per contenere tutte le cifre
	int n = 0; 
	for(x = 0; line[x] != '\0'; x++){
		/*se line[x] è una cifra mettila in s[n]
		if(isdigit(line[x])){ 
			s[n] = line[x];
			s[n+1] = '\0';
			n++;
...
/*Legge da file i parametri di configurazione*/
/*void load_info(int* parametri){
	...
	if ((fd = fopen("./config/config.txt", "r")) == NULL){
        	printf("Errore load_info\n");
        	exit(1);         
    	}
	/*reads text until newline
	while((fscanf(fd, "%[^\n]", line) != EOF)){
	    offset = ftell(fd) + 1; /*posizione attuale + 1
	    fseek(fd, offset , SEEK_SET); /*posizionati alla riga successiva
	    parametri[i] = extractNumber(line); /*array[i] viene riempito con il numero estratto dalla stringa
	    i++;
	}
}*/
